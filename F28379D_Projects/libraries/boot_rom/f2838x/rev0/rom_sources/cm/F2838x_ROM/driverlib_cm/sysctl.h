...
//
//*****************************************************************************
#define SYSCTL_CAUSE_PORN                0x00000001U //!< Power-on reset
...
#define SYSCTL_DELAY  __asm(  "    .sect \".text:SysCtl_delay\"\n" \
                              "    .clink\n"                       \
...
typedef enum
{
    ...
    SYSCTL_PERIPH_RES_TIMER0      = 0x0002U,  //!< CPUtimer0 Reset
...
//! - \b SYSCTL_CAUSE_PORN - Power-on reset
...
SysCtl_getResetCause(void)
{
 ...
 return(HWREG(CMSYSCTL_BASE + SYSCTL_O_CMRESC) &
       ((uint32_t)SYSCTL_CMRESC_POR | (uint32_t)SYSCTL_CMRESC_XRSN |
...
//! OR of \b SYSCTL_CAUSE_PORN, \b SYSCTL_CAUSE_XRSN, 
...
//! eg. SYSCTL_CMECATCTL_LOCK is for the CMECATCTL register
//!
...
SysCtl_disableEthercatI2Cloopback(void)
{
  ...
  //
  HWREGH(CMSYSCTL_BASE + SYSCTL_O_CMECATCTL)&= ~SYSCTL_ETHERCAT_I2C_LOOPBACK ;
}
