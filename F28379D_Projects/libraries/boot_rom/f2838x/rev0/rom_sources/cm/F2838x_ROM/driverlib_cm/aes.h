...
#include "debug.h"
#include "interrupt.h"
#include "inc_cm/hw_aes.h"
#include "inc_cm/hw_aes_ss.h"
...
//*****************************************************************************
//
// Mask to specify all the posiible DMA requests. It is the logical OR
...
//
//*****************************************************************************
#define AES_CONFIG_M  ((uint32_t)AES_S_AES_CTRL_DIRECTION   | \
                       (uint32_t)AES_S_AES_CTRL_MODE        | \
                       (uint32_t)AES_S_AES_CTRL_KEY_SIZE_M  | \
                       (uint32_t)AES_S_AES_CTRL_CTR         | \
                       (uint32_t)AES_S_AES_CTRL_CTR_WIDTH_M | \
                       (uint32_t)AES_S_AES_CTRL_ICM         | \
                       (uint32_t)AES_S_AES_CTRL_CFB         | \
                       (uint32_t)AES_S_AES_CTRL_XTS_M       | \
                       (uint32_t)AES_S_AES_CTRL_F8          | \
                       (uint32_t)AES_S_AES_CTRL_F9          | \
                       (uint32_t)AES_S_AES_CTRL_CBCMAC      | \
                       (uint32_t)AES_S_AES_CTRL_GCM_M       | \
                       (uint32_t)AES_S_AES_CTRL_CCM         | \
                       (uint32_t)AES_S_AES_CTRL_CCM_L_M     | \
                       (uint32_t)AES_S_AES_CTRL_CCM_M_M)
...
//!
//! This function performs a softreset of the AES module.
//!
//! \return None.
//
//*****************************************************************************
...
AES_performSoftReset(uint32_t base)
{
    ...
    // Trigger the reset.
    //
    HWREG(base + AES_O_S_AES_SYSCONFIG) |= AES_S_AES_SYSCONFIG_SOFTRESET;

    //
    // Wait for the reset to finish.
    //
    while((HWREG(base + AES_O_S_AES_SYSSTATUS) &
            AES_S_AES_SYSSTATUS_RESETDONE) != AES_S_AES_SYSSTATUS_RESETDONE)
    {
    }
}
...
AES_setDataLength(uint32_t base, uint64_t length)
{
    ...
    //
    HWREG(base + AES_O_S_AES_C_LENGTH_0) = (uint32_t)(length);
    HWREG(base + AES_O_S_AES_C_LENGTH_1) = (uint32_t)(length >> 32U);
}
...
AES_setAuthDataLength(uint32_t base, uint32_t authLength)
{
    ...
    //
    HWREG(base + AES_O_S_AES_AUTH_LENGTH) = authLength;
}
...
AES_enableInterrupt(uint32_t base, uint32_t wrapperBase, uint32_t intFlags)
{
    ...
    //
    HWREG(base + AES_O_S_AES_IRQENABLE) |= (intFlags & AES_AESINT_M);
...
AES_enableDMARequest(uint32_t base, uint32_t reqFlags)
{
    ...
    // Enable DMA requests.
    //
    HWREG(base + AES_O_S_AES_SYSCONFIG) |= reqFlags;
}
...
//!
//! \param base is the base address of the AES module.
//! \param AES_ConfigParams is the configuration of the AES module.
//!
...
//! configured.  The least significant word is at the 0th index.
...
//! significant word is in the 0th index.
//!
...
//! module is properly configured the key, data size, mode, and so on.  Only ECB,
//! CBC, CTR, ICM, CFB, XTS and F8 operating modes should be used.  The data
//! is processed in 4-word (16-byte) blocks.
//!
//! \note This function only supports values of \e dataLength less than 2^32,
