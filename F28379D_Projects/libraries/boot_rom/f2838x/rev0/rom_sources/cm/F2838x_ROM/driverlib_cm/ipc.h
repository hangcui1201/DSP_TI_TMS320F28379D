...
// FILE:   ipc.h
//
// TITLE:  Driver for the IPC module.
//
//###########################################################################
// $TI Release:  $
// $Release Date:  $
...
#define IPC_BUFFER_SIZE     4U   // No. of IPC messages in circular buffer
                                 // (must be interval of 2)
#define NUM_IPC_INTERRUPTS  4U   // No. of IPC interrupts using circular buffer
...
//
//*****************************************************************************
#define MAX_BUFFER_INDEX    (IPC_BUFFER_SIZE - 1U)
...
typedef struct
{
    IPC_Message_t Buffer[NUM_IPC_INTERRUPTS][IPC_BUFFER_SIZE];
    uint16_t      PutWriteIndex[NUM_IPC_INTERRUPTS];
    uint16_t      GetReadIndex[NUM_IPC_INTERRUPTS];
}IPC_PutBuffer_t;

typedef struct
{
    IPC_Message_t Buffer[NUM_IPC_INTERRUPTS][IPC_BUFFER_SIZE];
    uint16_t      GetWriteIndex[NUM_IPC_INTERRUPTS];
    uint16_t      PutReadIndex[NUM_IPC_INTERRUPTS];
}IPC_GetBuffer_t;
#endif
...
//! For example in case of F28307X device, the address spaces of C28x core and
...
//!
//! \return \b false if the queue is full. \b true if the message is successfully
//! sent.
//
//*****************************************************************************
extern bool
