...
//*****************************************************************************
//
//! Registers an interrupt handler for the McBSP receiver module.
//!
//! \param base specifies the McBSP module base address.
//! \param interruptfn is a pointer to the function to be called when the
//! McBSP interrupt occurs.
//!
//! This function registers the handler to be called when a McBSP interrupt
//! occurs.  This function enables the global interrupt in the interrupt
//! controller; specific McBSP interrupts must be enabled via
...
//*****************************************************************************
static inline void
McBSP_registerRxInterrupt(uint32_t base, void (*interruptfn)(void))
{
    uint32_t regVal;
    ...
    //
    //Check the McBSP module base address and assigns value of receive
    //interrupt to ui32Int
    //
    if(base == MCBSPA_BASE)
    {
        regVal = INT_MCBSPA_RX;
    }
    else
    {
        regVal = INT_MCBSPB_RX;
    }

    //
    // Registers a function to be called when interrupt occurs.
    //
    if(regVal != 0U)
    {
        Interrupt_register(regVal, interruptfn);
    }

    //
    // Enable the McBSP interrupt.
    //
    Interrupt_enable(regVal);
}

//*****************************************************************************
//
//! Registers an interrupt handler for the McBSP transmitter module.
//!
//! \param base specifies the McBSP module base address.
//! \param interruptfn is a pointer to the function to be called when the
//! McBSP interrupt occurs.
...
//*****************************************************************************
static inline void
McBSP_registerTxInterrupt(uint32_t base, void (*interruptfn)(void))
{
    uint32_t regVal;
    ...
    if(base == MCBSPA_BASE)
    {
        regVal = INT_MCBSPA_TX;
    }
    else
    {
        regVal = INT_MCBSPB_TX;
    }
