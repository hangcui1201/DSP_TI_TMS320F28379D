...
//*****************************************************************************
//
// A mapping of CLA base address to interrupt number. Not intended for use by
// application code.
//
//*****************************************************************************
...
CLA_getBackgroundTaskStatus(uint32_t base, CLA_BGTaskStatus stsFlag)
{
    ...
    switch(stsFlag)
    {
        ...
        {
            if((HWREGH(base + CLA_O_MSTSBGRND) & 0x1U) != 0U)
            {
                status = true;
            }
            break;
        ...
        {
            if((((HWREGH(base + CLA_O_MSTSBGRND) &
                    CLA_MSTSBGRND_BGINTM) >> 1U) & 0x1U) != 0U)
            {
                status = true;
            }
            break;
        ...
        {
            if((((HWREGH(base + CLA_O_MSTSBGRND) &
                     CLA_MSTSBGRND_BGOVF) >> 2U) & 0x1U) != 0U)
            {
                status = true;
            }
            break;
